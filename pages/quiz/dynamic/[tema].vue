<template>
    <Navbar />
    <div class="pointer-events-none text-end px-20 mt-4">
        <p class="text-2xl">Pontuação: {{ pontuacao }}</p>
    </div>
    <div class="flex flex-col p-20">
        <h3 class="text-2xl w-[600px] mb-6">
            Então quer dizer que acredita ser um especialista em {{ tema.nome }}, vamos ver do que é capaz, boa sorte
            marujo!!
        </h3>
        <div class="flex flex-col w-full gap-4">
            <h1 class="text-4xl">{{ tema.perguntas[i].pergunta }}</h1>
            <button :class="[
                'rounded-lg p-2 px-4 text-2xl h-12 w-full',
